...
if phase == '운영':
    ...
    print(f'[LOG] 피처마트 적재 코드 실행 기준년월 = {baseYM}, 장르 = {gnr}')
    ...
    print(f'''[LOG] 피처마트 적재 시작, 기준년월 = {baseYM}, 시작시간 = {datetime.strftime(dst_tm, '%Y%m%d %H:%M:%S')}''')
    ...
    lencnt  = pd.read_sql(f''' SELECT count(*) FROM FTRMART WHERE 기준년월 = {baseYM} ''',conn).values[0][0]

    dend_tm = datetime.now(KST)
    del_tm  = dend_tm-dst_tm
    print(f'''[LOG] 피처마트 적재 완료, 기준년월 = {baseYM}, 완료시간 = {datetime.strftime(dend_tm, '%Y%m%d %H:%M:%S')}, 소요시간 = {del_tm}''')
    print(f'[LOG] 피처마트 적재 코드 실행 완료, 기준년월 = {baseYM}, 장르 = {gnr}')